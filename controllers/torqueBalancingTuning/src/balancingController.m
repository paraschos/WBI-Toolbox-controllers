...
function [tauModel,Sigma,NA,f_HDot, ...
    ...
    if gain.activateGainTuning == 1
      
    gainDAngularMomentum  = MomGainOpt(4:6,4:6);
    gainPAngularMomentum  = intMomGainOpt(4:6,4:6);
    gainsPCOM             = intMomGainOpt(1:3,1:3);
    gainsDCOM             = MomGainOpt(1:3,1:3);
    impedances            = impedancesOpt;
    dampings              = dampingOpt;
    else        
    gainDAngularMomentum  = gain.DAngularMomentum.*eye(3);
    gainPAngularMomentum  = gain.PAngularMomentum.*eye(3);
    gainsPCOM             = gain.PCOM;
    gainsDCOM             = gain.DCOM;
    ...
    tauModel        = PInv_JcMinvSt*(JcMinv*h - JcDv) + nullJcMinvSt*(h(7:end) - Mbj'/Mb*h(1:6) ...
                       -impedances*qTilde  -ki_int_qtilde -dampings*qD);
    ...
    errorCoM                  = xcom - desired_x_dx_ddx_CoM(:,1);
     % control torques (no QP)
    tau                       =  tauModel+Sigma*f;
    
    % nonLinear joint accelerations
    dNu                       = M\(Jc'*f + [zeros(6,1); tau]-h);
    ddqjNonLin                = dNu(7:end);
